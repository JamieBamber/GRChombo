...
class ReprocessingLevel : public GRAMRLevel
{
	...
	// Do the extraction on the min integration level
	    ...
	    WeylExtraction weyl_extraction(m_p.extraction_params, m_p.output_rootdir, m_p.data_subdir, m_p.suffix,
					m_dt, m_time, m_first_step);
            weyl_extraction.execute_query(m_gr_amr.m_interpolator); //! <--- This routine includes performing the integration and writing the output to a file
        }		
