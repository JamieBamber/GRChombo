...
class ReprocessingLevel : public GRAMRLevel
{
	...
	BoxLoops::loop(
            RedefinedDensities(m_p.integration_params, m_dx, m_p.center),
	...
	// Do the extraction on the min integration level
	    ...
	    double rho_max = m_gr_amr.compute_max(Interval(c_rho, c_rho));
            double rho_azimuth_max = m_gr_amr.compute_max(Interval(c_rho_azimuth, c_rho_azimuth));

	    //int r_max = (int)(m_p.integration_params.max_integration_radius+0.5);
	    std::string rho_filename = m_p.output_rootdir + m_p.data_subdir + "_max_rho" + m_p.suffix; 
	    std::string rho_azimuth_filename = m_p.output_rootdir + m_p.data_subdir + "_max_rho_azimuth" + m_p.suffix;
            
	    // write out the integrals
	    write_integral(rho_max, rho_filename, m_first_step);
	    write_integral(rho_azimuth_max, rho_azimuth_filename, m_first_step);
        }		
    }
