...
struct integration_params_t
{
    ...
    double bh_a;	// dimensionfull Black Hole spin given by J/M or Ms where s = dimensionless spin
};
...
class SimulationParameters : public ChomboParameters
{
  public:
	...
	// load l m modes
        ...
        {
            pp.load("num_modes", integration_params.num_modes);
            std::vector<int> integration_modes_vect(2 *
                                                   integration_params.num_modes);
            pp.load("modes", integration_modes_vect,
                    2 * integration_params.num_modes);
            integration_params.modes.resize(integration_params.num_modes);
            for (int i = 0; i < integration_params.num_modes; ++i)
            {
             	integration_params.modes[i].first = integration_modes_vect[2 * i];
