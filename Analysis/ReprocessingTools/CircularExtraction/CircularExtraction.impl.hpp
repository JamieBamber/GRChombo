...
#if !defined(CIRCULAREXTRACTION_HPP_)
#error "This file should only be included through CircularExtraction.hpp"
#endif

#ifndef CIRCULAREXTRACTION_IMPL_HPP_
#define CIRCULAREXTRACTION_IMPL_HPP_

//! Set up and execute the interpolation query
inline void CircularExtraction::execute_query(
    AMRInterpolator<Lagrange<4>> *a_interpolator) const
{
    CH_TIME("CircularExtraction::execute_query");
...
//! integrate over a circle for each radius
inline std::vector<double>
CircularExtraction::integrate_surface(const std::vector<double> a_re_part) const
{
    CH_TIME("CircularExtraction::integrate_surface");
    int rank;
    #ifdef CH_MPI
	...
	for (int iradius = 0; iradius < m_params.num_integration_radii;
		...
		double theta = 0.5 * M_PI;
		int idx = iradius * m_num_points + iphi;
		...
		inner_integral_re += f_theta_phi_re;
		#ifdef _OPENMP
			#pragma omp atomic
		#endif
...
inline void
CircularExtraction::write_integral(const std::vector<double> a_integral,
                               std::string a_filename) const
{
    CH_TIME("CircularExtraction::write_integral");
...
inline void
CircularExtraction::write_extraction(std::string a_file_prefix,
                                 const std::vector<double> a_re_part) const
{
    CH_TIME("CircularExtraction::write_integration");
...
#endif /* CIRCULAREXTRACTION_IMPL_HPP_ */
