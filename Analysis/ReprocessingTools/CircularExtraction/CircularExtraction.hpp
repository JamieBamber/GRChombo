...
#ifndef CIRCULAREXTRACTION_HPP_
#define CIRCULAREXTRACTION_HPP_
...
class CircularExtraction
{
  ...
  public:
    //! The constructor
    CircularExtraction(integration_params_t a_params, double a_dt, double a_time,
                   bool a_first_step, std::string a_data_subdir, std::string a_output_rootdir, int a_start_number, int a_end_number, double a_restart_time = 0.0)
        : m_params(a_params), m_dt(a_dt), m_time(a_time),
          m_first_step(a_first_step), m_data_subdir(a_data_subdir), m_output_rootdir(a_output_rootdir), m_start_number(a_start_number), m_end_number(a_end_number),
	  m_restart_time(a_restart_time), m_num_points(m_params.num_points_phi),
    ...
    CircularExtraction(integration_params_t a_params, double a_dt, double a_time,
        ...
        : CircularExtraction(a_params, a_dt, a_time, (a_dt == a_time),
...
#include "CircularExtraction.impl.hpp"

#endif /* CIRCULAREXTRACTION_HPP_ */
