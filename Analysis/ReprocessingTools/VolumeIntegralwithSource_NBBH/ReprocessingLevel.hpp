...
class ReprocessingLevel : public GRAMRLevel
{
	...
	// Do the extraction on the min integration level
	    ...
	    double Cphi_sum = m_gr_amr.compute_sum(c_Cphi, m_p.coarsest_dx);
	    ...
	    std::string Cphi_filename = m_p.output_rootdir + m_p.data_subdir + "_Cphi" + m_p.suffix;
	    
	    // write out the integrals
	    write_integral((1+m_p.half_box)*rho_sum, mass_filename, m_first_step);
	    write_integral((1+m_p.half_box)*rho_J_sum, J_filename, m_first_step);
	    write_integral((1+m_p.half_box)*Cphi_sum, Cphi_filename, m_first_step);
	}		
