...
def make_rho_movie(dd):
	...
	SliceAtts.originIntercept = 0.5*L
	...
	AnnotationAtts.axes2D.xAxis.title.title = "y"
	...
	View2DAtts.windowCoords = (0.5*L-0.5*width, 0.5*L+0.5*width, 0-0.5*width, 0+0.5*width)
	...
	frame_dir = "BBH_GR_{:s}_rho_YZ_movie".format(dd.name)
	try:
