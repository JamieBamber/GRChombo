...
class data_dir:
	...
	def load_data(self):
		...
		self.inner_mass_flux = -flux_data[1:,1]/(E0) #*float(self.theta_max))
		self.outer_mass_flux = -flux_data[1:,2]/(E0) #*float(self.theta_max))	
		if cumulative:
...
#add_data_dir(5, 1, 1, "0.7", "0.4", "0", 64, 64, "0.97")
#add_data_dir(5, 1, 1, "0.7", "0.4", "0", 64, 64, "0.98")
add_data_dir(10, 8, 8, "0.7", "0.4", "0", 64, 18, "1.0")
add_data_dir(10, 8, 8, "0.7", "0.4", "0", 64, 64, "0.99")
#add_data_dir(5, 1, 1, "0.7", "0.4", "0", 32, 32, "1.0")
#add_data_dir(5, 1, 1, "0.7", "0.4", "0", 64, 32, "0.99")
#add_data_dir(5, 1, 1, "0.7", "0.4", "0", 64, 18, "1.0")
...
def plot_graph():
	...
	for dd in data_dirs:
		...
		label_ = "$N_{\\theta},N_{\\phi}=$" + "{:d},{:d}".format(dd.ntheta,dd.nphi) + " $\\theta_{max}=$"+dd.theta_max
		...
		ax1.plot(dd.tflux*mu,(10**5)*(dd.outer_mass_flux-dd.analytic_outer_flux),colours[i]+"-", label=label_, linewidth=1)
	...
	if cumulative:
		...
		save_path = home_path + "plots/mass_flux_in_R{:.0f}_IsoKerr_compare_theta_max_cumulative.png".format(R_max)
	else:
		ax1.set_ylabel("numerical - analytic flux / $E_0 \\times 10^{-5}$", fontsize=label_size)
		plt.title("Mass flux, $M=1$, $\\mu=0.4$, $\\chi=0.7$, $l=m=8$", wrap=True, fontsize=title_font_size)
		save_path = home_path + "plots/mass_flux_in_R{:.0f}_IsoKerr_compare_theta_max.png".format(R_max)
