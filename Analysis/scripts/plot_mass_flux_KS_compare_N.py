...
class data_dir:
	def __init__(self, num, l, m, a, mu, nphi, ntheta, suffix, N):
		...
		if N==128:
			...
			Nfix="_N{:d}".format(N)
		self.name = "run{:04d}_l{:d}_m{:d}_a{:s}_Al{:s}_mu{:s}_M1_KerrSchild{:s}".format(num, l, m, a, "0", mu, Nfix)
	#
	def load_data(self):
...
def add_data_dir(num, l, m, a, mu, nphi, ntheta, suffix, N=128):
	x = data_dir(num, l, m, a, mu, nphi, ntheta, suffix, N)
	data_dirs.append(x)
...
def plot_graph():
	...
	for dd in data_dirs:
		...
		label_ = "$l$={:d} $m$={:d} $N$={:d}".format(dd.l, dd.m, dd.N)
	...
	if cumulative:
		...
		save_path = home_path + "plots/mass_flux_Kerr_Schild_cumulative_compare_N_r_plus_to_500.png"
		...
		save_path = home_path + "plots/mass_flux_Kerr_Schild_compare_N_r_plus_to_500.png"
