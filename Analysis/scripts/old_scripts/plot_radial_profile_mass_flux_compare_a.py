...
class data_dir:
	...
	#
	def load_data(self):
		file_name = self.name+"_J_R_Y00_integral_{:s}_r_plus_to_{:d}_nphi{:d}_ntheta{:d}_theta_max{:.1f}.dat".format(scale, R_max, self.nphi, self.ntheta, self.theta_max)
...
def plot_graph():
	...
	dd0 = data_dirs[0]
	tau=dd0.time*dd0.mu
	title = "Mass flux profile $M=1,\\mu=0.4,l=m=1,\\tau=${:.1f} \n averaged over 5 timesteps".format(tau) 
	ax1.set_title(title, fontsize=title_font_size)
	plt.tight_layout()
	if log_y:
			save_name = "/home/dc-bamb1/GRChombo/Analysis/plots/IsoKerr_J_R_profile_{:s}_Rmax={:d}_tau={:.1f}_compare_a_log_y.png".format(scale, R_max, tau)
	else:
			save_name = "/home/dc-bamb1/GRChombo/Analysis/plots/IsoKerr_J_R_profile_{:s}_Rmax={:d}_tau={:.1f}_compare_a.png".format(scale, R_max, tau)
