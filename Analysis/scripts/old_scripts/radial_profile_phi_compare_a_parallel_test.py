...
# sphere weighting field = (cell_volume) / (2*pi * r^2 * dr) 
# slice weighting field = (cell_volume)^(2/3) / (2*pi * r * dr) 
if sphere_or_slice:
...
### make plot
number = int(full_number/plot_interval)
dsi = ds0[number]
...
for i in range(0,len(data_dirs)):
	if (i != min_index):
		#dsi1 = yt.load(data_root_path + "/" + data_dirs[i].name + "/KerrSFp_{:06d}.3d.hdf5".format(number))
		dsi1 = datasets[i][number]
		plot_profile(dsi1, i)			
	else:
		pass
plt.xlabel("$r_*$")
