...
def load_flux_data():
	...
	for dd in data_dirs:
		...
		file_name = home_path + output_dir + "/" + dd.name + "_J_rKS_linear_n000000_r_plus_to_{:d}_nphi{:d}_ntheta{:d}{:s}_g_munu_norm.dat".format(r_max, dd.nphi, dd.ntheta, dd.suffix)
		data_g_munu_norm[dd.num] = np.genfromtxt(file_name, skip_header=1)
		print("loaded flux data for " + file_name)
	return (data, data_g_munu_norm) 	
...
def plot_graph():
	flux_data, flux_data_g_munu_norm = load_flux_data()
	if plot_mass:
	...
	for dd in data_dirs:
		...
		flux_line_data_g_munu_norm = flux_data_g_munu_norm[dd.num]
		mu = float(dd.mu)
		tflux = flux_line_data[1:,0]
		tflux_g_munu_norm = flux_line_data_g_munu_norm[1:,0]
		...
		inner_mass_flux_g_munu_norm = -flux_line_data_g_munu_norm[1:,1]/E0
		outer_mass_flux_g_munu_norm = -flux_line_data_g_munu_norm[1:,2]/E0
		if average_time:
		...
		if cumulative:
			...
			inner_mass_flux_g_munu_norm = np.cumsum(inner_mass_flux_g_munu_norm)*dt
			outer_mass_flux_g_munu_norm = np.cumsum(outer_mass_flux_g_munu_norm)*dt
		...
		#ax1.plot(tflux_g_munu_norm,inner_mass_flux_g_munu_norm,colours2[i]+"--", label="flux into BH ($g^{ab} N_a N_b=1$ norm) "+label_)# r={:.1f} ".format(r_min)+label_)
		ax1.plot(tflux_g_munu_norm,outer_mass_flux_g_munu_norm,colours2[i]+"-.", label="flux into r={:.1f}".format(r_max)+" ($g^{ab} N_a N_b=1$ norm) "+label_)
		ax1.plot(tflux,analytic_outer_flux,"k-.", label="4th order t$\\mu$/r analytic flux into r={:.1f} ".format(r_max)+label_) #+" times 4$\\pi$")
		...
		if plot_mass:
			...
			ax1.plot(tmass,delta_mass/E0,colours[i]+"-", label="change in mass $r_+<r<${:.1f} ".format(r_max)+label_)
		i = i + 1
	ax1.set_xlabel("$t$")
	#ax1.set_xlim((0, 300))
	...
	if cumulative:
		...
		save_path = home_path + "plots/mass_flux_Kerr_Schild_cumulative_run0105_r_plus_to_500_compare_norm.png"
	else:
		ax1.set_ylabel("flux / $E_0$")
		plt.title("Mass flux")
		save_path = home_path + "plots/mass_flux_Kerr_Schild_run0105_r_plus_to_500_compare_norm.png"
