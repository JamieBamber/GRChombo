...
add_data_dir(10, 8, 8, "0.7", "0.4", "0", 264, 264, "_theta_max0.99")
...
def plot_graph():
	...
	for dd in data_dirs:
		...
		label_ = "$l$={:d} $m$={:d} ".format(dd.l, dd.m) + "$N_{\\phi}=N_{\\theta}=$" + "{:d}".format(dd.nphi)
		#label_ = "$m$={:d} $\\alpha$={:s}".format(dd.m, dd.Al)
		#ax1.plot(dd.tflux,dd.inner_mass_flux,colours[i]+":", label="flux into $R_+$ "+label_)
		ax1.plot(dd.tflux*dd.mu,dd.outer_mass_flux,colours[i]+"-", label=label_, linewidth=1)
		ax1.plot(dd.tflux*dd.mu,dd.analytic_outer_flux,colours[i]+"--", label="_4th order t$\\mu$/r analytic flux into R={:.1f} ".format(R_max)+label_, linewidth=1)
		#
		if plot_mass:
			if cumulative:
				ax1.plot(dd.tmass*dd.mu,dd.dmass,colours[i]+"-.", label="_change in mass $R_+<R<${:.1f} ".format(R_max)+label_, linewidth=1)
			elif not cumulative:
				ax1.plot(dd.tmass*dd.mu,dd.dmass,colours[i]+"-.", label="_rate of change in mass $R_+<R<${:.1f} ".format(R_max)+label_, linewidth=1)
		i = i + 1
	ax1.set_xlabel("$\\tau$", fontsize=label_size)
	#ax1.set_xlim((0, 200))
	ax1.set_ylim((0, 0.000001))
	if cumulative:
		ax1.set_ylabel("cumulative flux / $E_0$", fontsize=label_size)
		ax1.set_title("Cumulative mass flux, $M=1$, $\\mu=0.4$", wrap=True, fontsize=title_font_size)
		save_path = home_path + "plots/mass_flux_in_R{:.0f}_IsoKerr_compare_NphiNtheta_cumulative.png".format(R_max)
	else:
		ax1.set_ylabel("flux / $E_0$", fontsize=label_size)
		plt.title("Mass flux, $M=1$, $\\mu=0.4$, $l=1$")
		save_path = home_path + "plots/mass_flux_in_R{:.0f}_IsoKerr_compare_NphiNtheta.png".format(R_max)
