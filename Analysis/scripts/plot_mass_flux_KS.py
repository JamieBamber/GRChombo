...
class data_dir:
	def __init__(self, num, l, m, a, mu, nphi, ntheta):
		self.num = num
		self.l = l
		self.m = m
		self.a = float(a)
...
data_dirs = []		
def add_data_dir(num, l, m, a, mu, nphi, ntheta):
	x = data_dir(num, l, m, a, mu, nphi, ntheta)
	data_dirs.append(x)
...
def load_data():
	...
	ang_flux_data = {}
	for dd in data_dirs:
		file_name = home_path + output_dir + "/" + dd.name + "_J_rKS_linear_n000000_nphi{:d}_ntheta{:d}.dat".format(dd.nphi, dd.ntheta)
...
def plot_graph():
	...
	if cumulative:
		...
		save_path = home_path + "plots/mass_flux_Kerr_Schild_nphi{:d}_ntheta{:d}.png".format(dd.nphi, dd.ntheta)
