...
for i in range(0, len(a_list)):
	...
	x = r
	plt.plot(r, phi, colours[i], markersize=2, label="a = " + a_list[i])
	###
	...
	def Stationary_sol(r, phase):
		...
		r_factor = M*(r_plus*np.log(r/r_plus - 1) - r_minus*np.log(r - r_minus))/(r_plus - r_minus) 
		result = -A*(HR*np.sin(omega*(t-r_factor)-phase) - HI*np.cos(omega*(t-r_factor)-phase))
		return result	
	r_out = r[15:]
	phi_out = phi[15:]
	#popt, pconv = curve_fit(Stationary_sol, r_out, phi_out, p0=(0))
	popt = [0.14]
	phi_fit = Stationary_sol(r_out, popt[0])
	plt.plot(r_out, phi_fit, colours2[i], linewidth=1, label="stationary solution, a={:s}, A={:.2f} phase={:.2}".format(a, A, popt[0]))		
plt.ylabel("$\\phi$")
...
save_name = "phi_profile_vs_r_star_compare_Heun_KS_t={:.2f}.png".format(t)
