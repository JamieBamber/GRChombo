...
BinaryBH_fixed_dir = "run0007_FlatScalar_mu1_G0_delay1500"
BinaryBH_dir = "run0005_FlatScalar_mu1_G0"

movie_folder = "BBH_run0007/Compare_ray_profile_phi_run0005_run0007_movie"
try:
...
def plot_BH_arr(n_BinaryBH, BBH_data, KBH_data, ax, colors, adlabel, single_BH_label=True):
	# get dataslice
	...
	if single_BH_label:
		label_start=""
		ax.plot(line_pos, arrK1, colors[0]+'--', label=label_start+"single BH", linewidth=line_width)
		ax.plot(line_pos, arrK2, colors[0]+'--', label="_single BH 2", linewidth=line_width)
		ax.plot(line_pos, 0.5*(arrK1+arrK2), colors[1]+'-', label=label_start+"mean of single BHs", linewidth=line_width)
	else:
		label_start="_"
	ax.plot(line_pos, arrB, colors[2]+'-', label="Binary BHs "+adlabel, linewidth=1)
	phi_max=np.max(arrB)
	phi_min=np.min(arrB)	
...
for n_BinaryBH in range(60, 61):
	...
	plot_BH_arr(n_BinaryBH, BBH_fixed_data, KBH_data, ax, ['y', 'm', 'g'], "fixed metric", False)
	plot_BH_arr(n_BinaryBH, BBH_data, KBH_data, ax, ['c', 'b', 'r'], "")

	title = "Compare 1D ray $\\phi$ profiles \n for binary and two single BHs, t={:.1f}".format(t)
	plt.legend(fontsize=legend_font_size, ncol=2)
	plt.title(title, fontsize=title_font_size)
	ax.set_xlabel("line position from centre", fontsize=label_size)
	ax.set_ylabel("$\\phi$", fontsize=label_size)
