...
def plot_graph():
	...
	if cumulative:
		if diff_from_alpha0:
			...
			ax1.set_title("Cumulative mass flux, $M=1,\\mu=0.4$,\n$\\chi=0.99,l=|m|=1$", wrap=True, fontsize=title_font_size)			
			save_path = home_path + "plots/plots_for_first_paper/Fig_15_mass_flux_in_R{:.0f}_IsoKerr_compare_Al_cumulative.pdf".format(R_max)
	else:
		if diff_from_alpha0:
			if log_y:
