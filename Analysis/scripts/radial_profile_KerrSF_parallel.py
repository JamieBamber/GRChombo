...
# output file
output_file = data_sub_dir + "_radial_profile" + ".csv"
...
for sto, dsi in ds.piter(storage=data_storage):
	...
	# make slice 
	slice = dsi.[:,:,z_position]
	# restrict to within the maximum radius
	# circle = slice.cut_region(["obj['cylindrical_radius'] < r_max"])	
	
	# make profile
	radia_profile = yt.create_profile(slice, ["cylindrical_radius"], fields=["rho"], 

	radial_profile = yt.Profile1D(slice, "cylindrical_radius", 
