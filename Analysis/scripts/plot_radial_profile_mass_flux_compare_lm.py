...
def plot_graph():
	...
	dd0 = data_dirs[0]
	tau=dd0.time*dd0.mu
	title = "Mass flux profile $M=1,\\mu=0.4,\\chi=0.7,\\tau=${:.1f}".format(tau) 
	ax1.set_title(title, fontsize=title_font_size)
	plt.tight_layout()
	if log_y:
			save_name = "/home/dc-bamb1/GRChombo/Analysis/plots/IsoKerr_J_R_profile_{:s}_Rmax={:d}_tau={:.1f}_compare_lm_log_y.png".format(scale, R_max, tau)
	else:
			save_name = "/home/dc-bamb1/GRChombo/Analysis/plots/IsoKerr_J_R_profile_{:s}_Rmax={:d}_tau={:.1f}_compare_lm.png".format(scale, R_max, tau)
