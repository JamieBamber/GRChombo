...
movie_folder = "BBH_run0010/Compare_ray_profile_rho_run0010_run0012_movie"
try:
...
def plot_BH_arr(n_BinaryBH, BBH_data, ax, colors, adlabel):
	...
	ax.plot(line_pos, np.log10(arrB), colors[2]+'-', label="Binary BHs "+adlabel, linewidth=1)
	phi_max=np.max(arrB)
	phi_min=np.min(arrB)	
...
for n_BinaryBH in range(54, 55):
	...
	title = "Compare 1D ray $\\rho$ profiles for unequal binary \n ratio 2 for fixed and free metric, t={:.1f}".format(t)
	plt.legend(fontsize=legend_font_size)
	ax.set_title(title, wrap=True, fontsize=title_font_size)
	ax.set_xlabel("line position from centre", fontsize=label_size)
	ax.set_ylabel("$\\log_{10}(\\rho)$", fontsize=label_size)
