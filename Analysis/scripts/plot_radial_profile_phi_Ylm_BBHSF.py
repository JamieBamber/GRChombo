...
for i in range(0, len(a_list)):
	...
	for j in range(0, len(lm_list)):
		...
		r = R*(1 + r_plus/(4*R))**2
		if log_x:
			plt.plot(np.log10(R), phi_integral, colours[j] + styles[i], label="$a = ${:.2f} $l,m = $ {:d},{:d} mode".format(a, l, m))
		else:	
			plt.plot(R, phi_integral, colours[j] + styles[i], label="$a = ${:.2f} $l,m = $ {:d},{:d} mode".format(a, l, m))
if log_x:
