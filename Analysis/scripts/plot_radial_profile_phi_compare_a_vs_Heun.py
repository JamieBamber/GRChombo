        ...
        for i in range(0, r.size):
                if (r[i]<=r_plus):
                        sol[i] = 0
                     	...
                     	sol[i] = Kerrlib.Rfunc(M, mu, omega, a, 0, 0, ingoing, False, t - 2*np.pi*phase/omega, r[i])
        sol = np.abs(A)*sol
        return sol
...
for i in range(0, len(a_list)):
	...
	if (a==0):
		...
		popt, pconv = curve_fit(Stationary_sol_KS_fit, r, phi, p0=(10, 0))
		phi_fitted = Stationary_sol_KS_fit(r, popt[0], popt[1])		
		plt.plot(x, phi_fitted, 'k--', linewidth=1, label="KS stat. sol., a=0, amplitude={:.2f} phase={:.2}".format(popt[0], popt[1]))		
plt.ylabel("$\\phi_{00}/\\phi_0$")
plt.legend(fontsize=8)
#plt.xlim((0, 100))
#plt.ylim((-7.5, 7.5))
title = "$\\Phi$ profile, Isotropic Kerr coordinates, $M\mu=M\omega=0.4$ $l=m=0$ time={:.1f}".format(t) 
plt.title(title)
if lin_or_log:
	plt.xlabel("$r_{BL}/M$")
else:
...
save_name = "phi_profile_compare_a_vs_Heun_{:s}_t={:.1f}_rmax_100.png".format(scale, t)
