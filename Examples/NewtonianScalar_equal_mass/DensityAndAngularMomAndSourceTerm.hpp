...
#ifndef DENSITYANDANGULARMOMANDSOURCETERM_HPP_
#define DENSITYANDANGULARMOMANDSOURCETERM_HPP_
...
template <class matter_t, class background_t> class DensityAndAngularMomAndSourceTerm
{
  ...
  public:
    DensityAndAngularMomAndSourceTerm(matter_t a_matter, background_t a_background,
                                double a_dx,
	...
	// Now get source term
	// delta_mu Phi = lapse delta_mu lapse; 
	// C_mu = (delta_mu lapse) * ( rho / lapse - det_gamma ^ {2/3} * S );
	Tensor<1, data_t> Ci;
	FOR1(i) { Ci[i] = metric_vars.d1_lapse[i] * ( rho / metric_vars.lapse - pow(det_gamma, 2/3) * emtensor.S ); }
	// that was in cartesian directions
	data_t Cphi;
	FOR1(i) { Cphi += Ci[i]*dxdphi[i]; }
	// 
	...
	current_cell.store_vars(Cphi, c_Cphi);
    }
};

#endif /* DENSITYANDANGULARMOMANDSOURCETERM_HPP_ */
