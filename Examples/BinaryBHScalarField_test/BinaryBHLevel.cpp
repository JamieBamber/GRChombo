...
void BinaryBHLevel::initialData()
{
    ...
    // Initial conditions for field - constant amplitude
    double VofPhi = 1; // just set the value to be order 1 (amend G to vary)
    double phi_amplitude =
        m_p.field_amplitude *
        sqrt(VofPhi / (0.5 * pow(m_p.potential_params.scalar_mass, 2.0)));

    // Initial conditions for scalar field - constant amplitude
    BoxLoops::loop(SetValue(phi_amplitude, Interval(c_phi, c_phi)), m_state_new,
...
void BinaryBHLevel::postRestart()
{
    ...
    if (m_p.track_punctures == 1 && m_level == 0)
    {
        ...
        if (m_time > 0.0)
        {
            ...
            AMRInterpolator<Lagrange<4>> interpolator(m_bh_amr, m_p.origin,
                                                      m_p.dx, m_p.verbosity);
            m_bh_amr.set_interpolator(&interpolator);

            // read in punctures
            PunctureTracker my_punctures(m_time, m_restart_time, m_dt,
                                         m_p.checkpoint_prefix);
            my_punctures.read_in_punctures(m_bh_amr);
        }
...
void BinaryBHLevel::specificPostTimeStep()
{
    ...
    // do puncture tracking on finest level but one
