...
class NewtonianBHFixedBG
{
  public:
    ...
    NewtonianBHFixedBG(const params_t a_params, const double a_dx,
                       const double a_time = 0.0, const double a_separation = 0.0,
                       const double a_omega = 0.0)
      : m_params(a_params), m_dx(a_dx), m_time(a_time), m_omega(a_omega), m_separation(a_separation)
    {
    }
                                   ...
                                   const Cell<data_t> &current_cell) const
    {
        ...
        if (m_time != 0.0)
        {
            ...
            center_now[0] =
                m_params.center[0] +
	      sign * 0.5 * m_separation * (cos(m_omega * m_time) - 2);
