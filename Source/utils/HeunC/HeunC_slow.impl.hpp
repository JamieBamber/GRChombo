                          ...
                          std::complex<double> delta_, std::complex<double> eta_, double z)
{
  ...
  if (z>=1){
    ...
    if (std::abs(z-1)<Heun_proxco){
      std::cout << "|z-1| < " << Heun_proxco << " --> using HeunCnear1" << std::endl;
      std::pair<HeunCvars, HeunCvars> vars_vars1 = HeunCnear1(p,z);
      result = vars_vars1.first;
    }
    else if ( (std::abs(p.epsilon)>1/2)&& (std::abs(p.q) < 2.5) && (std::abs(z)>Heun_proxcoinf_rel*R/(std::abs(eps)+std::abs(p.epsilon))) ) {
      std::pair<HeunCvars, HeunCvars> vars1_vars = HeunCfaraway(p,z);
      result = vars1_vars.second;
    }
    else {
      std::cout << "--> using HeunC0" << std::endl;
      result = HeunC0(p,z);
    }
    return result; 
  }
...
inline HeunCvars HeunC::HeunC0(HeunCparams p, double z, bool aux){
  ...
  if (z>=1){
    ...
    if (std::abs(z)<Heun_cont_coef) {
        std::cout << "HeunC0: result <-- HeunC00 		z = " << z << std::endl;
	result = HeunC00(p1,z,aux);
    }
    else {
      double z0 = Heun_cont_coef*z/std::abs(z);
      std::cout << "HeunC0: result0 <-- HeunC00 		z0 = " << z0 << std::endl;
      HeunCvars result0 = HeunC00(p1,z0,aux);
      std::cout << "HeunC0: result1 <-- HeunCconnect 	z0 = " << z0 << " z = " << z << std::endl;
...
inline HeunCvars HeunC::HeunCfromZ0(HeunCparams p,double z,double Z0,std::complex<double> H0,std::complex<double> dH0)
{
  ...
  double r = std::min(std::abs(Z0),std::abs(Z0-1.0));
  
  if (std::abs(z-Z0)>=r) {
    std::cout << "HeunCfromZ0: z = " << z << " z0 = " << Z0 << " r = " << r << std::endl;
   ...
   result.err= 0; result.numb = 0;
    ...
    if ( isinf(std::abs(result.val)) || isinf(std::abs(result.dval)) || isnan(std::abs(result.val)) || isnan(std::abs(result.dval)) ) {
      ...
      if (p.q-p.alpha*z != 0) {
        val2 = ( z*(z-1)*ddval+(p.gamma*(z-1)+p.delta*z+p.epsilon*z*(z-1))*result.dval ) / (p.q-p.alpha*z);
        std::cout << "HeunCfromZ0: val2 = " << val2 << std::endl;
        err1 = std::abs(result.val-val2);
      }
      else {
        err1 = INFINITY;
  ...
  std::cout << "HeunCfromZ0:	 val = " << result.val << " err = " << result.err << " numb = " << result.numb << std::endl;
  return result; 
}
...
inline HeunCvars HeunC::HeunCconnect(HeunCparams p,double z, double z0,std::complex<double> H0,std::complex<double> dH0, double R0, bool aux)
{
  ...
  if ((z==0)||(z==1)||(z0==0)||(z0==1)){
    ...
    double R = std::min(Rmax,std::min(std::abs(z0),std::abs(z0-1))*Heun_cont_coef);

    // first set of iteration variables
    bool Rtuned = false;
    int iter = 1;
    double z1;       // step end point
    double z0_ = z0; // step start point
    int positivity = 2*(z >= z0_) - 1;
    ...
    while (Rtuned==false) {
      if (std::abs(z-z0_) <= R) {
        ...
        std::cout << "positivity = " << positivity << std::endl;
        z1 = z0_ + R * positivity;
      }
      
      std::cout << "HeunCconnect: HeunCfromZ0 <-- 	z1 = " << z1 << " z0 = " << z0_ << " H0 = " << H0 << " dH0 = " << dH0 << " R = " << R << std::endl;
      result = HeunCfromZ0(p,z1,z0_,H0,dH0);

      Rtuned = (result.err < 5*eps) && (result.numb < Heun_optserterms) || (iter>5) || (result.numb<=8);
      
      if (Rtuned==false) {
        std::cout << "*** HeunCconnect: tuning R" << std::endl;
        R = R / std::max(result.err/(5*eps), static_cast<double>(result.numb)/Heun_optserterms);
      }
      std::cout << "HeunCfromZ0(at end of first loop) R = " << R << std::endl;
    ...
    while (insearch) {
      R = std::min(R, std::min(std::abs(z0_),std::abs(z0_-1))*Heun_cont_coef);
      if (std::abs(z-z0_) <= R) {
        z1 = z; insearch = false;
      }
      else {
      ...
      std::cout << "HeunCconnect(loop 2): HeunCfromZ0 <-- 	z0_ = " << z0_ << " z1 = " << z1 << " H0 = " << H0 << " dH0_ " << dH0 <<" R = " << R << std::endl;      
      result_ = HeunCfromZ0(p,z1,z0_,H0_,dH0_);
      H0_ = result_.val;
      dH0_ = result_.dval;
    ...
    std::cout << "HeunCconnect:		result val = " << result.val << " err = " << result.err << " numb = " << result.numb << std::endl;
  }
  return result;
}
...
inline std::pair<HeunCvars, HeunCvars> HeunC::HeunCnear1(HeunCparams p,double z)
{
  ...
  HeunCvars vars1f = HeunC1(p,z);
  HeunCvars vars1s = HeunCs1(p,z);
  ...
  std::pair<HeunCvars, HeunCvars> output(vars1, vars2);
  return output;
}
...
inline std::pair<HeunCvars, HeunCvars> HeunC::HeunCfaraway(HeunCparams p,double z)
{
  ...
  if (z>=1) {
    ...
    std::complex<double> aSt = I/p.epsilon;
...
inline ConnectionVars HeunC::HeunCjoin10(HeunCparams p)
{
  ...
  if (consts_known) {
    ...
    result.err = vars0.err/(1+std::abs(vars0.val)) + vars0s.err/(1+std::abs(vars0s.val)) + vars1.err/(1+std::abs(vars1.val)) + vars1s.err/(1+std::abs(vars1s.val));
    result.numb = vars0.numb + vars0s.numb + vars1.numb + vars1s.numb;
  
    Matrix<std::complex<double>> m, b, c;
    ...
    c = m.dot((result.C10).transpose())-b; 
    result.err += std::abs(c(0, 0)) + std::abs(c(1, 0)) + std::abs(c(0, 1)) + std::abs(c(1, 1)); 
...
inline ConnectionVars HeunC::extrdatfromsav(HeunCparams p, std::vector<savedataVars> savedata, bool& consts_known){
  ...
  if (savedata.size() !=0) {
    for(int k=1; k < savedata.size(); k++){
        s = savedata[k];
	HeunCparams ps = s.p;
        if (ps.q == p.q && ps.alpha == p.alpha && ps.gamma == p.gamma && ps.delta == p.delta && ps.epsilon == p.epsilon) {
        	result = s.Cvars;        
        	consts_known = "true";
        	break;
      	}
