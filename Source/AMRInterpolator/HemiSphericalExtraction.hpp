...
#include "HemiSphericalGeometry.hpp"
#include "HemiSphericalHarmonics.hpp"
#include "SurfaceExtraction.hpp"

//! A child class of SurfaceExtraction for extraction on spherical shells
class HemiSphericalExtraction : public SurfaceExtraction<HemiSphericalGeometry>
{
  public:
    ...
    HemiSphericalExtraction(const params_t &a_params, double a_dt, double a_time,
    ...
    HemiSphericalExtraction(const params_t &a_params,
                        const std::vector<vars_t> &a_vars, double a_dt,
                        double a_time, bool a_first_step,
                        double a_restart_time = 0.0)
        : HemiSphericalExtraction(a_params, a_dt, a_time, a_first_step,
                              a_restart_time)
    {
        add_vars(a_vars);
    }
        ...
        const IntegrationMethod &a_method_phi = IntegrationMethod::trapezium)
    {
        auto integrand_re = [center = m_center, &geom = m_geom, es, el, em,
            ...
            HemiSphericalHarmonics::Y_lm_t<double> Y_lm =
                HemiSphericalHarmonics::spin_Y_lm(x, y, z, es, el, em);
